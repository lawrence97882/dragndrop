...
class dragndrop{
    // 
    //The element on the svg that is selectable.The selectable elements are defines 
    //in the method is selectable any extensions are done there. This property is optional 
    //since it is not set during construction but only selectable elements with a class of 
    //draggable can be selected.
    public selected_element?: SVGElement;
    public selected_group ?: SVGGElement;
    // 
    //The coordinates at the start of the drag and drop process which is the same as the 
    //mouse position at the begining of the drag.
    public coord_start?:number
    //
       ...
       if(selectable===true){
            ...
            if(document.querySelectorAll('[selected=true]')=== null || document.querySelectorAll('[selected=true]') === undefined){
             ...
             for(let i=0; i<last_selections.length; i++){
              ...
              const selected_element= (<HTMLOrSVGImageElement>e.target);
              //
              //Set a new selected element
    ...
    get_mouse_position(e:MouseEvent):number {
       //
       //
   ...
   start_drag(e:Event){
       //
       //Ensure that we can only drag an element that has the class draggble.
       //Anything else is ignored.
       if (!(<HTMLOrSVGImageElement>e.target).classList.contains('draggable')) return;
        //
        //The selected element that is suppost to be dragged 
        this.selected_element= <SVGEllipseElement>e.target; 
         //
        //get the parent group tag <g> to be draged
        this.selected_group= <SVGGElement>this.selected_element.parentElement;
        //
        //Set the mouse position at the start of the draf position in svg 
        //coordinatees
   ...
   //Incase we want to add a new element to to select add its description here 
   is_selectable(e:Event){
       //
       //An element is selectable if it is either a line or an ellipse
       //
       //Test if the element is a line 
        if ((<HTMLOrSVGImageElement>e.target).nodeName==="line"){
            return true;
       }
       //
       //test if the element is an ellipse
       if ((<HTMLOrSVGImageElement>e.target).nodeName==="ellipse"){
            return true;
       }
       //
